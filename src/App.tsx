...
import { readFileToRawData, guessMapping, processStudentDataWithMapping } from './services/dataProcessor';
import { generateStudentMessage } from './services/geminiService';
import { Student, GeneratedMessage, GenerationStyle, ColumnMapping } from './types';
...
const App: React.FC = () => {
  ...
  const [mappingFile, setMappingFile] = useState<{file: File, rawData: any[][], mapping: ColumnMapping} | null>(null);
  ...
  useEffect(() => {
    const processQueue = async () => {
        if (pendingFiles.length > 0 && !mappingFile) {
            const nextFile = pendingFiles[0];
            setIsProcessingFiles(true);
            try {
                const rawData = await readFileToRawData(nextFile);
                const guessedMapping = guessMapping(rawData);
                setMappingFile({
                    file: nextFile,
                    rawData,
                ...
                console.error(`Failed to read file: ${nextFile.name}`, e);
                alert(`שגיאה בקריאת הקובץ: ${nextFile.name}`);
                // Remove faulty file and continue
    ...
    processQueue();
  }, [pendingFiles, mappingFile]);

  const handleFilesSelected = (files: File[]) => {
    // Filter duplicates against uploaded AND pending
    const newFiles = files.filter(f => 
        !uploadedFiles.some(uf => uf.name === f.name) && 
        !pendingFiles.some(pf => pf.name === f.name)
    );
    
    if (newFiles.length === 0) {
        if (files.length > 0) alert("הקבצים שבחרת כבר קיימים במערכת");
        return;
    }
  ...
  const handleMappingConfirmed = (finalMapping: ColumnMapping) => {
      ...
      const newStudents = processStudentDataWithMapping(
          mappingFile.rawData, 
          finalMapping, 
          mappingFile.file.name,
          students // Merge with existing
      );
      
      setStudents(newStudents);
      setUploadedFiles(prev => [...prev, mappingFile.file]);
      
      // Cleanup: Clear mapping file and remove from queue
      setMappingFile(null);
      setPendingFiles(prev => prev.slice(1));
  };
  ...
  const handleRemoveFile = (fileName: string) => {
      if(confirm("הסרת קובץ תאפס את כל הנתונים כרגע (בגרסה זו). האם להמשיך?")) {
          setStudents([]);
          setUploadedFiles([]);
          setMessages({});
      }
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 sm:p-8 font-sans text-slate-800 flex flex-col">
      <div className="max-w-[1600px] mx-auto space-y-8 flex-grow w-full">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-8 items-start">
            ...
            <div className="xl:col-span-1 space-y-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    ...
                    />
                    {isProcessingFiles && <p className="text-sm text-indigo-600 mt-3 text-center font-medium animate-pulse">פותח חלון מיפוי...</p>}
                    {pendingFiles.length > 0 && !isProcessingFiles && (
                         <p className="text-xs text-slate-500 mt-2 text-center">ממתינים למיפוי: {pendingFiles.length} קבצים</p>
                    )}
                </div>
        ...
        {mappingFile && (
            <ColumnMapperModal 
                fileName={mappingFile.file.name}
                rawData={mappingFile.rawData}
                initialMapping={mappingFile.mapping}
